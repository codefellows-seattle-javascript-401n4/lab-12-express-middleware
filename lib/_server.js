...
app.post('/api/notes', jsonParser, (req, res, next) => {
  ...
  if(! req.body.title) next(createError(400, 'Please enter title'));
  if(! req.body.content) next(createError(400, 'Please enter content'));
...
app.put('/api/notes', jsonParser, (req, res, next) => {
  ...
  if(id) {
    ...
    Note.updateNote(id, req.body)
      ...
      .catch(() => next(createError(400, 'Please use valid ID')));
  }

  else next(createError(400, 'No ID try again'));
